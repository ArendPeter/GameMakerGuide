...
This isn't exactly a standard pong feature, but it did feel like the next logical step in your Game Maker journey so ima force it
...
> NOTE: I added ``// oPaddle Draw Event`` to clarify the event, but you shouldn't need to use the comment (the event should already be clear). Now that you're comfortable with events I can do this to make my descriptions a little shorter
...
In larger games, the name could be referenced 1000s of times across the whole experience, and at that point entering the string directly into our functions will become tedious. It would be nice if we could set the name in one spot and then refer back to it whenever we need it...
...
Some of you probably hit errors on that previous assignment (I'm actually hoping that some of you did, that's the best way to learn 😊 )
...
This means you likely copied the draw event code over without copying the code in the create event. It can't find the ``name`` variable because the variable was never created
 ...
 * **Usage/Reference**: This is where you're using a variable but not changing it's value, you're using (or referencing) it. This happens whenever you include variables on the right side of the assignment operator, or when you use variables as inputs to functions
...
## Allowing the player to input names

Some of you may be disappointed that I used the RPG name input as an example, but didn't show you how to actually allow players to input custom names
